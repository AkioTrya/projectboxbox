...
const TrackMap: React.FC<TrackMapProps> = ({ 
    ...
    const [trackData, setTrackData] = useState<{ points: Point[], sector_boundaries: any } | null>(null);
    ...
    const getXYAtProgress = (progress: number) => {
        ...
        const segmentProgress = distDiff === 0 ? 0 : (targetDist - p1.dist) / distDiff;
    ...
    if (!trackData) return <div className="flex items-center justify-center h-full text-f1-gray animate-pulse font-display text-xs">INITIALIZING TRACK TELEMETRY...</div>;
                    ...
                    <g transform={`translate(${offset.x}, ${offset.y}) scale(${zoom})`} style={{ transformOrigin: 'center' }}>
                        ...
                        {driverPositions.map(pos => (
                            <g 
                                ...
                                onClick={(e) => { e.stopPropagation(); setSelectedDriver(pos.driver); }}
                            >
                                <circle 
                                    r={selectedDriver === pos.driver ? 10 / Math.sqrt(zoom) : 6 / Math.sqrt(zoom)} 
                                    fill={pos.color} 
                                    className={selectedDriver === pos.driver ? '' : 'animate-pulse'} 
                                    style={{ 
                                ...
                                <text 
                                    ...
                                    style={{ fontSize: `${10 / zoom}px`, textShadow: '0 1px 2px rgba(0,0,0,0.8)' }}
                                >
                                    {pos.driver}
                                </text>
                            </g>
                ...
                <div className="absolute top-4 left-4 bottom-4 w-72 bg-pit-black/90 backdrop-blur-2xl border border-pit-border/50 p-5 shadow-2xl flex flex-col gap-6 animate-in slide-in-from-left duration-500 z-50">
                    <div className="flex justify-between items-start">
                        <div className="flex flex-col">
                            <span className="text-[10px] text-f1-gray font-display font-bold tracking-widest uppercase">{focusedDriver.team}</span>
                            <h2 className="text-3xl font-display font-black tracking-tighter italic" style={{ color: focusedDriver.color }}>{focusedDriver.driver}</h2>
                        </div>
                        <button onClick={() => setSelectedDriver(null)} className="text-f1-gray hover:text-f1-white">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-pit-dark/50 p-3 border border-pit-border/30 rounded-sm">
                            <span className="text-[8px] text-f1-gray uppercase font-display block mb-1">Current Lap</span>
                            <span className="text-xl font-mono font-bold">{focusedDriver.lap}</span>
                        </div>
                        <div className="bg-pit-dark/50 p-3 border border-pit-border/30 rounded-sm">
                            <span className="text-[8px] text-f1-gray uppercase font-display block mb-1">Tyre Strategy</span>
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: TYRE_COLORS[focusedDriver.compound?.toUpperCase()] || '#555' }}></div>
                                <span className="text-lg font-mono font-bold">{focusedDriver.compound?.charAt(0)}{focusedDriver.tyre_life}</span>
                            </div>
                        </div>
                    </div>

                    <div className="flex flex-col gap-3">
                        <span className="text-[10px] text-f1-gray uppercase font-display tracking-widest font-bold">Stint History</span>
                        <div className="flex flex-col gap-1 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                            {focusedDriver.allLaps.slice(0, focusedDriver.lap).reverse().map((l: any, i: number) => (
                                <div key={i} className="flex justify-between items-center text-[10px] font-mono p-1.5 border-b border-pit-border/20">
                                    <span className="text-f1-gray">L{l.lap_number}</span>
                                    <span className={l.is_personal_best ? 'text-f1-purple' : 'text-f1-white'}>
                    ...
                    <div className="mt-auto pt-4 border-t border-pit-border/30">
                        <div className="flex justify-between items-center text-[10px] uppercase font-display text-f1-gray mb-2">
                            ...
                            <span className="text-f1-white">{((focusedDriver.progress) * 100).toFixed(1)}%</span>
                        </div>
                        <div className="h-2 bg-pit-border/20 rounded-full overflow-hidden">
                            <div className="h-full transition-all duration-300" style={{ width: `${focusedDriver.progress * 100}%`, backgroundColor: focusedDriver.color }}></div>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-3">
                    {driverPositions.map((pos, idx) => {
                        return (
                            <div 
                                key={pos.driver} 
                                onClick={() => setSelectedDriver(pos.driver)}
                                className={`bg-pit-dark/90 border p-2.5 flex flex-col gap-2 shadow-lg transition-all hover:border-f1-white/30 cursor-pointer ${selectedDriver === pos.driver ? 'border-f1-white scale-[1.02] z-10 ring-1 ring-f1-white/20' : 'border-pit-border/80'}`}
                            >
                                <div className="flex justify-between items-center border-b border-pit-border/40 pb-1.5">
                                    <div className="flex items-center gap-2">
                                        <span className="text-[9px] font-mono text-f1-gray w-3">{idx + 1}</span>
                                        <div className="w-1 h-3.5 rounded-full" style={{ backgroundColor: pos.color }}></div>
                                        <span className="font-display font-bold text-[11px] uppercase tracking-wider">{pos.driver}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div 
                                            className="w-2.5 h-2.5 rounded-full border border-white/20" 
                                            style={{ backgroundColor: TYRE_COLORS[pos.compound?.toUpperCase()] || '#555' }}
                                            title={`${pos.compound} - ${pos.tyre_life} Laps`}
                                        ></div>
                                        <span className="text-[10px] text-f1-gray font-display font-bold">L{pos.lap}</span>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-1.5">
                                    {[1, 2, 3].map(s => {
                                        ...
                                        const time = s === 1 ? pos.s1 : s === 2 ? pos.s2 : pos.s3;
                                            ...
                                            <div key={s} className="flex flex-col gap-1">
                                                <div className="h-1 bg-pit-border/30 rounded-full overflow-hidden">
                                                    <div 
                                                        ...
                                                        style={{ 
                                                            width: isDone ? '100%' : isCurrent ? `${((pos.progress % 0.33) / 0.33) * 100}%` : '0%',
                                                            backgroundColor: isDone ? '#52E252' : isCurrent ? pos.color : 'transparent'
                                                        }}
                                                    ></div>
                                                </div>
                                                <div className="font-mono text-[9px] text-f1-white font-bold text-center">
                                                    {isDone ? (time ? (time/1000).toFixed(2) : '--.--') : isCurrent ? 'RUN' : '--.--'}
                                                </div>
                                            </div>
                                        );
                                    })}
                                ...
                                <div className="flex justify-between items-center text-[8px] font-display uppercase tracking-widest text-f1-gray/60">
                                    <span>TYRE: {pos.tyre_life || '0'}L</span>
                                    <span style={{ color: idx === 0 ? '#52E252' : 'inherit' }}>
                                        {idx === 0 ? 'INTERVAL' : `+${pos.gap.toFixed(3)}s`}
                                    </span>
                                </div>
                            </div>
                        );
