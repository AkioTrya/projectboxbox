            ...
            <div className="bg-pit-dark border border-pit-border p-4 font-display text-xs shadow-2xl backdrop-blur-md bg-opacity-95 min-w-[200px]">
                ...
                <div className="flex flex-col gap-2">
                    {payload.map((entry: any) => {
                        ...
                        return (
                            <div key={entry.dataKey} className="flex flex-col gap-1 border-b border-pit-border/30 pb-1 last:border-0">
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-2">
                                        <div 
                                            className={`w-2 h-2 ${lapData?.is_personal_best ? 'bg-green-500' : 'bg-red-600'}`} 
                                            title={lapData?.is_personal_best ? "Personal Best" : "No Personal Best"}
                                        ></div>
                                        <span style={{ color: entry.color }} className="font-bold">{entry.name}</span>
                                    </div>
                                    <span className="text-f1-white font-mono">
                                        {focus === 'pace' ? formatLapTime(entry.value) : 
                                         focus === 'dist' ? `${entry.value} Laps` :
                                    ...
                                    <div className="flex justify-between items-center pl-4 text-[9px] uppercase tracking-tighter opacity-70">
                                        <div className="flex items-center gap-1">
                                            <div 
                                                ...
                                                style={{ backgroundColor: TYRE_COLORS[lapData.compound?.toUpperCase()] || '#fff' }}
                                            ></div>
...
function OnboardingModal({ onComplete }: { onComplete: (themeId: string, focus: string) => void }) {
            ...
            <div className="bg-pit-panel border border-pit-border p-8 max-w-2xl w-full flex flex-col gap-8 shadow-2xl">
                <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-3">
                        ...
                        <p className="text-f1-red font-display tracking-widest text-xs uppercase">Welcome to Pit Wall</p>
                    </div>
                    <h2 className="text-4xl font-display font-bold tracking-tight uppercase">Personalize Your Dashboard</h2>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="flex flex-col gap-4">
                        <label className="text-xs font-display text-f1-gray tracking-widest uppercase">Select Your Identity</label>
                            ...
                            {Object.values(THEMES).map((t) => (
                                <button
                                    ...
                                    className={`p-4 border font-display text-[10px] uppercase tracking-widest transition-all ${
                                        theme === t.id ? 'border-f1-white bg-f1-white/10' : 'border-pit-border hover:border-f1-gray'
                                    }`}
                                    style={{ borderColor: theme === t.id ? t.accent : '' }}
                                >
                                    <div className="w-full h-1 mb-2" style={{ backgroundColor: t.accent }}></div>
                                    {t.name}
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-col gap-4">
                        <label className="text-xs font-display text-f1-gray tracking-widest uppercase">Select Your Focus</label>
                        <div className="flex flex-col gap-2">
                            <button
                                onClick={() => setFocus("pace")}
                                className={`p-4 border font-display text-[10px] uppercase tracking-widest text-left transition-all ${
                                    focus === "pace" ? 'border-f1-white bg-f1-white/10' : 'border-pit-border hover:border-f1-gray'
                                }`}
                            >
                                <p className="font-bold mb-1">Pace Analysis</p>
                                <p className="text-[9px] opacity-60">Lap times and relative performance.</p>
                            </button>
                            <button
                                onClick={() => setFocus("dist")}
                                className={`p-4 border font-display text-[10px] uppercase tracking-widest text-left transition-all ${
                                    focus === "dist" ? 'border-f1-white bg-f1-white/10' : 'border-pit-border hover:border-f1-gray'
                                }`}
                            >
                                <p className="font-bold mb-1">Pace Distribution</p>
                                <p className="text-[9px] opacity-60">Consistency and lap time spread.</p>
                            </button>
                            <button
                                onClick={() => setFocus("tyre")}
                                className={`p-4 border font-display text-[10px] uppercase tracking-widest text-left transition-all ${
                                    focus === "tyre" ? 'border-f1-white bg-f1-white/10' : 'border-pit-border hover:border-f1-gray'
                                }`}
                            >
                                <p className="font-bold mb-1">Tyre Strategy</p>
                                <p className="text-[9px] opacity-60">Degradation vs Tyre Age analysis.</p>
                            </button>
                        </div>
                    </div>
                </div>
...
function DashboardContent() {
    ...
    useEffect(() => {
        if (sessionId) {
            ...
            fetch(`/api/laps?session_id=${sessionId}`)
                ...
                .then(data => {
                    setLaps(data || []);
                    const drivers = Array.from(new Set((data || []).map((l: any) => l.driver))) as string[];
    ...
    const chartData = useMemo(() => {
        if (focus === "dist") {
            const filteredLaps = laps.filter(lap => lap.lap_time_ms > 0 && lap.lap_time_ms <= maxLapTime);
        ...
        const map: any = {};
        filteredLaps.forEach(lap => {
            if (!map[lap.lap_number]) map[lap.lap_number] = { lap: lap.lap_number, rawData: {} };
    ...
    if (!sessionId) {
        return (
            <div className="flex-1 bg-pit-black text-f1-white flex flex-col items-center justify-center p-8 text-center">
                <div className="w-16 h-1 mb-6 animate-pulse" style={{ backgroundColor: theme?.accent || '#e8002d' }}></div>
                <h2 className="text-2xl font-display font-bold mb-4 uppercase tracking-widest">No Session Selected</h2>
                <p className="text-f1-gray max-w-md mb-8">
                    To view telemetry data, please select a session from the Seasons menu or use the quick selector below.
                </p>
                <select 
                    onChange={(e) => router.push(`/dashboard?session_id=${e.target.value}`)}
                ...
                >
                    ...
                    {sessions.map((s) => (
                        <option key={s.id} value={s.id}>{s.year} {s.circuit} — {s.session_type}</option>
                    ))}
                </select>
            </div>
        );
    ...
    return (
        <div className="flex-1 flex overflow-hidden bg-pit-black text-f1-white">
            {showOnboarding && <OnboardingModal onComplete={handleOnboardingComplete} />}
            
            {/* Sidebar Controls */}
            <aside className="w-80 border-r border-pit-border bg-pit-panel p-6 overflow-y-auto flex flex-col gap-8 shadow-xl relative z-10">
                {/* Session Header */}
                <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: theme?.accent || '#e8002d' }}></div>
                        <span className="font-display text-[10px] tracking-widest uppercase" style={{ color: theme?.accent || '#e8002d' }}>Telemetria Active</span>
                    </div>
                    ...
                    <p className="text-f1-gray text-[10px] font-display tracking-widest uppercase">
                        {currentSession ? `${currentSession.year} • ${currentSession.session_type}` : "---"}
                    </p>
                </div>
                ...
                {(focus === "pace" || focus === "dist") && (
                    <div className="flex flex-col gap-4 p-4 border border-pit-border bg-pit-dark shadow-inner">
                        <div className="flex justify-between items-center">
                            <label className="text-[10px] font-display font-bold text-f1-white tracking-widest uppercase">Filter Threshold</label>
                            <span className="font-display text-[10px]" style={{ color: theme?.accent || '#e8002d' }}>{maxLapTime / 1000}S</span>
                        </div>
                        <input 
                            type="range" 
                            min="60000" 
                            ...
                            className="w-full h-1 bg-pit-border rounded-lg appearance-none cursor-pointer"
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex justify-between items-center">
                        <label className="text-[10px] font-display font-bold text-f1-white tracking-widest uppercase">Grid Selection</label>
                        <div className="flex gap-3">
                            <button onClick={selectAllDrivers} className="text-[9px] font-display text-f1-gray hover:text-f1-white uppercase tracking-tighter transition-colors">All</button>
                            <button onClick={clearAllDrivers} className="text-[9px] font-display text-f1-gray hover:text-f1-white uppercase tracking-tighter transition-colors">None</button>
                        </div>
                    </div>
                    <div className="grid grid-cols-3 gap-1.5">
                        {driversInSession.map((driver) => (
                            <button
                                ...
                                style={{
                                    borderColor: activeDrivers.includes(driver) ? DRIVER_COLORS[driver] || '#ffffff' : '#2a2a2a',
                                    color: activeDrivers.includes(driver) ? '#ffffff' : '#4b5563',
                                    backgroundColor: activeDrivers.includes(driver) ? `${DRIVER_COLORS[driver]}1a` : 'transparent'
                                }}
                ...
                {loading ? (
                    <div className="flex-1 flex items-center justify-center">
                        <div className="flex flex-col items-center gap-4">
                            <div className="w-12 h-12 border-4 border-pit-border border-t-f1-red rounded-full animate-spin" style={{ borderTopColor: theme?.accent }}></div>
                            <p className="text-f1-gray font-display text-xs tracking-[0.2em] uppercase">Syncing Data...</p>
                        </div>
                    </div>
                ) : laps.length > 0 ? (
                    <>
                        <div className="flex justify-between items-end border-b border-pit-border pb-6">
                            ...
                            <div className="flex gap-10 text-right">
                                <div className="flex flex-col gap-1">
                                    <p className="text-f1-gray text-[9px] font-display uppercase tracking-widest">Active Theme</p>
                                    <p className="font-display text-xs uppercase" style={{ color: theme?.accent || '#e8002d' }}>{theme?.name || "Boxbox Standard"}</p>
                                </div>
                                <div className="flex flex-col gap-1">
                                    <p className="text-f1-gray text-[9px] font-display uppercase tracking-widest">Data Points</p>
                                    <p className="text-f1-white font-display text-xs">{laps.length}</p>
                                </div>
                            </div>
                        </div>

                        <div className="flex-1 min-h-0 bg-pit-panel/20 border border-pit-border p-6 shadow-2xl relative">
                                ...
                                {focus === "dist" ? (
                                    <BarChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
                                        ...
                                        <YAxis 
                                            ...
                                            fontSize={10} 
                                            tick={{ fontFamily: 'var(--font-display)' }}
                                            label={{ value: 'LAP COUNT', angle: -90, position: 'insideLeft', fill: '#444', fontSize: 9, fontFamily: 'var(--font-display)' }}
                                        />
                                    ...
                                    <LineChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
                                        ...
                                        <XAxis 
                                            ...
                                            fontSize={10} 
                                            tick={{ fontFamily: 'var(--font-display)' }}
                                            label={{ value: 'LAP', position: 'bottom', offset: 0, fill: '#444', fontSize: 9, fontFamily: 'var(--font-display)' }}
                                        />
                                        <YAxis 
                                            stroke="#444" 
                                            fontSize={10} 
                                            tick={{ fontFamily: 'var(--font-display)' }}
                                            tickFormatter={focus === "pace" ? formatLapTime : (v) => `${v}L`}
                                            domain={['auto', 'auto']} 
                                            width={60} 
                                        />
                                        <Tooltip content={<CustomTooltip focus={focus} />} />
                    ...
                    <div className="flex-1 flex items-center justify-center border border-dashed border-pit-border rounded-lg bg-pit-panel/5">
                        <p className="text-f1-gray font-display text-xs uppercase tracking-[0.3em] italic opacity-40">No Telemetry Signal Found</p>
                    </div>
                )}
            </main>
        </div>
    ...
    return (
        <Suspense fallback={
            <div className="flex-1 bg-pit-black flex items-center justify-center">
                <p className="text-f1-gray font-display tracking-widest uppercase animate-pulse">Establishing Connection...</p>
            </div>
        }>
            <DashboardContent />
        </Suspense>
